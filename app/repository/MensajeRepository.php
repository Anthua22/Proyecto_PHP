...
class MensajeRepository extends QueryBuilder
{
    ...
    public function getMensajeChat(Mensajes $mensaje){
        $sql = "select * from ".$this->getTable().' where receptor = '.$mensaje->getReceptor().' and emisor = '.$mensaje->getEmisor(). ' or receptor = '.$mensaje->getEmisor().' and emisor ='.$mensaje->getReceptor(). ' order by hora asc;';
